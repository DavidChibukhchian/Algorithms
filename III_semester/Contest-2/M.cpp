...
int main()
{
	...
	size_t edges_count = (vertex_count * (vertex_count - 1)) / 2;

	// adding edges from the fictitious vertex
	edges_count += vertex_count;

	Graph graph(vertex_count + 1, edges_count);
	...
	for (size_t i = 0; i < vertex_count; i++)
	{
		size_t weight = 0;
		std::cin >> weight;
		graph.add_edge(vertex_count, i, weight);
	}
